...
- 위험하거나 협박을 받고 있으면 앱 사용보다 `112`, `1366`, 중앙디지털성범죄피해자지원센터 연결이 먼저입니다.
- 피해 이미지나 영상 원본은 올리거나 다시 내려받지 마세요.
- 비밀번호, 주민등록번호, 카드번호는 적지 마세요.
- URL, 게시 위치, 게시자 단서, 발견 일시만 차분히 정리하면 됩니다.
- 지움AI는 자동 삭제를 하지 않습니다. 대신 사용자가 공식 경로에 제출할 문서와 순서를 만들어 줍니다.

처음 사용하는 사람은 [첫 사용자 안내서](docs/first-user-guide-v1.8.md)를 먼저 읽으면 됩니다. 무엇을 눌러야 하는지, 무엇을 적지 말아야 하는지, 결과 화면을 어떻게 읽는지 1분 흐름으로 정리했습니다.
...
- 본인 계정·관리자 권한·대리 지원·사진 속 당사자·권한 불명확 상태를 분리하는 삭제 권한 게이트
...
- "내 사진"이라는 주장만으로 직접 삭제를 실행하지 않습니다. 본인 계정이나 관리자 권한이 확인되지 않으면 요청서와 공식기관 경로만 제공합니다.
...
앱을 처음 쓰는 사람은 [첫 사용자 안내서](docs/first-user-guide-v1.8.md)를 먼저 읽으면 됩니다.

프로젝트 구조를 이해하고 싶은 사람은 [초보자용 프로젝트 해설](docs/beginner-project-guide-v1.6.md)을 읽으면 됩니다. 개발자가 아니어도 “어느 파일을 보면 무엇을 알 수 있는지” 이해할 수 있도록 적었습니다.

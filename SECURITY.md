...
- 급한 협박이나 신변 위험이 있으면 앱 입력보다 `112`, `1366`, 전문기관 연결이 먼저입니다.
- 공용 PC에서는 사건을 저장하지 않는 편이 안전합니다.
- 피해 URL은 자동으로 열리지 않습니다. 사용자가 직접 확인할 때도 필요한 범위만 확인하세요.
- "내 사진"이라는 사실만으로 직접 삭제 권한이 생기지는 않습니다. 권한이 불명확하면 삭제 요청과 공식기관 경로만 사용하세요.

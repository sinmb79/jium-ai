...
describe("operational action plan", () => {
  it("turns a blocked handoff summary into redacted owner-routed actions", async () => {
    ...
    expect(plan.phases.find((phase) => phase.id === "go-live")?.actions.some((action) => action.action.includes("ops:launch-inputs:receipt"))).toBe(true);
    ...
    expect(plan.phases.find((phase) => phase.id === "desktop-release")?.actions.some((action) => action.action.includes("desktop:release-upload:check"))).toBe(true);

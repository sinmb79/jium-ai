...
const PHASE_BLUEPRINTS = [
  ...
  },
  {
    ...
    baseActions: [
      ...
      "Verify the uploaded GitHub Release assets with npm run desktop:release-upload:check after the publish job completes.",
    ],
    completionCriteria: [
  ...
  },
  {
    ...
    baseActions: [
      ...
      "Build the redacted approval command packet with npm run ops:approvals:command-packet before legal approval recording.",
      "Build the private-fill approval inputs template with npm run ops:approvals:input-template before collecting reviewer references.",
      "After real approval, apply the filled private input file with npm run ops:approvals:apply-inputs -- --input <private-approved-operational-inputs.json> --init.",
  ...
  },
  {
    ...
    baseActions: [
      ...
      "Run npm run ops:launch-inputs:apply-check to find missing private artifacts and env prerequisites before generating commands.",
      "Generate the private launch command packet with npm run ops:launch-inputs:commands so operators do not copy raw approved values into public reports.",
      ...
      "When using the Netlify MCP upload lane, run the generated deploy command from dist/static-hosting-bundle/site after the bundle is built.",

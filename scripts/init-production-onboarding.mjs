...
function onboardingReadme({ generatedAt, packageVersion }) {
  return [
    ...
    "7. `npm run ops:onboarding:approve-checklist -- --record <checklist-record-id> --evidence-ref <pseudonymous-evidence-reference>`",
    "8. Set `JIUM_HOSTED_SECURITY_HEADER_AUDIT_REPORT=ops/private/production-onboarding/hosted-security-header-audit.json` in the private go-live environment.",
    "9. `npm run server:deployment:bundle`",
    "10. `npm run desktop:publish:check -- --feed-dir <signed-desktop-folder>`",
    "11. `npm run ops:approvals:check`",
    "12. `npm run ops:go-live:check`",
    "13. `npm run ops:handoff:bundle`",
    "",
    "## Safety Rules",
    "",
